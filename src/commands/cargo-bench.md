...
基准过滤参数 _benchname_ 以及双横线（`--`）之后的所有参数，都会传给基准测试二进制，
并进一步传给 _libtest_（rustc 内置的单元测试与微基准框架）。
如果你同时给 Cargo 和二进制传参，`--` 后面的参数传给二进制，前面的传给 Cargo。
关于 libtest 参数，参见 `cargo bench -- --help` 输出以及 rustc 文档中测试机制章节：
<https://doc.rust-lang.org/rustc/tests/index.html>。

例如，下面命令只运行名为 `foo` 的基准（不会运行类似 `foobar` 的其他基准）：

    cargo bench -- foo --exact

基准测试通过给 `rustc` 传入 `--test` 来构建，这会把你的代码与 libtest 链接成一个
特殊可执行文件。该可执行文件会自动运行所有标注了 `#[bench]` 属性的函数。
Cargo 会向测试 harness 传入 `--bench`，表示只运行基准测试，不论该 harness 是
libtest 还是自定义 harness。

可以在 target 的 manifest 设置中将 `harness = false` 来禁用 libtest harness。
这种情况下，你的代码需要自行提供 `main` 函数来处理基准执行。

> **注意**：
> [`#[bench]` 属性](https://doc.rust-lang.org/nightly/unstable-book/library-features/test.html)
> 目前仍不稳定，仅在
> [nightly channel](https://doc.rust-lang.org/book/appendix-07-nightly-rust.html)
> 可用。你也可以在 stable 上使用一些来自
> [crates.io](https://crates.io/keywords/benchmark)
> 的基准库，例如 [Criterion](https://crates.io/crates/criterion)。

默认情况下，`cargo bench` 使用 [`bench` profile]，它会启用优化并关闭调试信息。
如果需要调试基准，可用 `--profile=dev` 切到 dev profile，
之后就可以在调试器中运行带调试信息的基准。

[`bench` profile]: ../reference/profiles.html#bench

### 基准测试工作目录

每个基准测试的工作目录都会被设置为其所属 package 的根目录。
这样无论 `cargo bench` 从哪里执行，基准测试都能稳定地通过相对路径访问 package 文件。
...
<dd class="option-desc"><p>只编译，不运行基准测试。</p>
</dd>
...
<dd class="option-desc"><p>无论是否失败都运行全部基准。若不指定该标志，Cargo 会在第一个可执行文件失败后退出。
Rust 测试 harness 会把单个可执行文件中的所有基准都跑完；该标志仅作用于“可执行文件”这一层。</p>
</dd>
...
<dd class="option-desc"><p>仅对指定 package 执行基准。SPEC 格式见 <a href="cargo-pkgid.html">cargo-pkgid(1)</a>。
...
<dd class="option-desc"><p>对 workspace 的全部成员执行基准。</p>
</dd>
...
当未给出 target 选择选项时，`cargo bench` 会为所选 package 构建以下 target：

- lib（用于与二进制及基准测试链接）
- bins（仅当会构建 benchmark targets 且所需 feature 可用时）
- 作为 benchmark 构建的 lib
- 作为 benchmark 构建的 bins
- benchmark targets

默认行为可通过 target 的 manifest 设置中的 `bench` 标志修改。
将 example 设置为 `bench = true` 会将该 example 作为基准构建并运行，
并用 libtest harness 替换 example 的 `main` 函数。

将 target 设置为 `bench = false` 可让其默认不参与基准。
按名称接收 target 的选择选项（如 `--example foo`）会忽略 `bench` 标志，
始终对给定 target 执行基准。

按 target 配置的更多信息见
[配置 target](../reference/cargo-targets.html#configuring-a-target)。

如果要执行基准的对象中包含集成测试或 benchmark，二进制 target 会自动构建。
这使集成测试可执行二进制以验证其行为。
当集成测试被构建和运行时，会设置
`CARGO_BIN_EXE_<name>`
...
传入 target 选择标志将仅对指定 target 执行基准。
...
<dd class="option-desc"><p>对该 package 的库执行基准。</p>
</dd>
...
<dd class="option-desc"><p>对指定示例执行基准。此标志可多次指定，并支持常见 Unix glob 模式。</p>
</dd>
...
<dd class="option-desc"><p>对所有示例 target 执行基准。</p>
</dd>
...
<dd class="option-desc"><p>对指定集成测试执行基准。此标志可多次指定，并支持常见 Unix glob 模式。</p>
</dd>
...
<dd class="option-desc"><p>对所有在 manifest 中设置了 <code>test = true</code> 的 target 执行基准。
...
<dd class="option-desc"><p>对指定 benchmark target 执行基准。此标志可多次指定，并支持常见 Unix glob 模式。</p>
</dd>
...
<dd class="option-desc"><p>对所有在 manifest 中设置了 <code>bench = true</code> 的 target 执行基准。
...
<dd class="option-desc"><p>对全部 target 执行基准。等价于指定 <code>--lib --bins --tests --benches --examples</code>。</p>
</dd>
...
<dd class="option-desc"><p>为指定目标架构执行基准。该标志可多次指定。默认是主机架构。
...
<dd class="option-desc"><p>使用给定 profile 执行基准。
...
默认情况下，Rust 测试 harness 会隐藏基准执行输出，以保持结果可读性。
若要恢复输出（例如调试），可给基准二进制传入 `--no-capture`：
...
该选项必须出现在命令名之前，例如 <code>cargo -C path/to/my-project bench</code>。</p>
...
`--jobs` 只影响基准可执行文件的构建，不影响运行基准时的线程数。
Rust 测试 harness 会在单线程中串行运行基准。
...
如需尽量运行更多基准而不是首个失败即停止，请使用 `--no-fail-fast`。
如需尽量“编译”更多基准，可先用 `--benches` 分开构建基准二进制，例如：
...
1. 构建并执行当前 package 的所有基准：

       cargo bench

2. 在指定 benchmark target 中只运行某个基准：
